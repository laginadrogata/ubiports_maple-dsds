...
// KPI tag defined in frameworks/base/services/core/java/com/android/server/am/EventLogTags.logtags
#define EVENT_LOG_TAG_kpi 2734
#define KPI_EVENT_NBR_PARAMETERS 3
#define KPINAME "dex2oat"
#define EVENT_BUFFER_SIZE 200
#define MAX_OAT_LOCATION_SIZE 100
...
static std::string arg_oat_location;

static void kpilog (uint64_t time) {
  const char* name = KPINAME;
  const char* oatfile = NULL;
  uint8_t buffer[EVENT_BUFFER_SIZE];
  int len_oatfile = 0;
  int idx = 0;
  int len_name = strlen(name);

  oatfile = arg_oat_location.c_str();
  len_oatfile = strlen(oatfile);
  if (len_oatfile > MAX_OAT_LOCATION_SIZE) {
    len_oatfile = MAX_OAT_LOCATION_SIZE;
  }

  // Define the KPI event
  buffer[idx++] = EVENT_TYPE_LIST;
  buffer[idx++] = KPI_EVENT_NBR_PARAMETERS;

  // Parameter 1: name
  buffer[idx++] = EVENT_TYPE_STRING;
  memcpy(&(buffer[idx]), &len_name, sizeof(int));
  idx += sizeof(int);
  memcpy(&(buffer[idx]), name, len_name);
  idx += len_name;

  // Parameter 2: action
  buffer[idx++] = EVENT_TYPE_STRING;
  memcpy(&(buffer[idx]), &len_oatfile, sizeof(int));
  idx += sizeof(int);
  memcpy(&(buffer[idx]), oatfile, len_oatfile);
  idx += len_oatfile;

  // Parameter 3: value
  // Convert time from ns to ms.
  time = time / 1000000;
  buffer[idx++] = EVENT_TYPE_LONG;
  memcpy(&(buffer[idx]), &time, sizeof(uint64_t));
  idx += sizeof(uint64_t);

  // Terminate to avoid exposing bugs
  buffer[idx++] = 0;

  // Send the event
  android_bWriteLog(EVENT_LOG_TAG_kpi, buffer, idx);
}
...
class Dex2Oat FINAL {
  ...
  void ParseArgs(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
      ...
      if (option.starts_with("--dex-file=")) {
        ...
        arg_oat_location = oat_location_;
 ...
 private:
  ...
  void LogCompletionTime() {
    ...
    uint64_t exectime = NanoTime() - start_ns_;
    LOG(INFO) << "dex2oat took "
              << PrettyDuration(exectime)
