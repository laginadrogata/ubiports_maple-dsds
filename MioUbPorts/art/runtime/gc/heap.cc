...
#if defined(GC_DUMP)
void Heap::DumpGcForAllocInfo(std::ostream& os, Thread* self, const std::string& gc_cause,
    bool isDumpAllocationStack) {
  std::stringstream sstr;
  sstr << gc_cause << "\n";
  int fd = open("/proc/self/cmdline", 0x0000);
  if (0 <= fd) {
    char procName[129];
    memset(procName, 0, sizeof(procName));
    if (read(fd, procName, sizeof(procName) - 1) != 1) {
        // Declared with attribute warn_unused_result
    }
    close(fd);
    sstr << "process name: " << procName << "\n";
    if (self == nullptr) {
      sstr << "now not attached to runtime!\n";
    } else {
      self->DumpJavaStack(sstr);
    }
  } else {
      sstr << "can't open cmdline!\n";
  }
  if (isDumpAllocationStack) {
    DumpAllocationStackInfo(sstr);
  }
  // we use nanotime for unique identifier of gc for alloc log
  const uint64_t time = NanoTime();
  std::ostringstream temp;
  temp << "GC for Alloc(" << time << ") dump: ";
  const std::string prefix = temp.str();
  std::string str;
...
void Heap::DumpAllocationStackInfo(std::ostream& os) {
  std::map<std::string, int> counter;
  const int MAX_COUNT = 1000;
  auto* start = allocation_stack_->Begin();
  auto* end = allocation_stack_->End();
  start = (start > end - MAX_COUNT) ? start : end - MAX_COUNT;
  for (auto* it = start; it < end; ++it) {
    std::string class_name(SafePrettyTypeOf(it->AsMirrorPtr()));
    counter[class_name]++;
  }
  int num = counter.size();
  std::string *keys = new std::string[num];
  int count = 0;
  for (std::map<std::string,int>::iterator i = counter.begin(); i != counter.end(); i++) {
      keys[count++] = i->first;
  }
  std::sort(keys, keys + count, [&] (const std::string& s1, const std::string& s2)
      {return counter[s1] > counter[s2];});
  os << "Allocation stack size: " << allocation_stack_->Size()<< "\n";
  for (int i = 0; i < num; i++) {
    os << "count " << keys[i] << ": " << counter[keys[i]] << "\n";
  }
  delete[] keys;
}
#endif
...
#if defined(GC_DUMP)
  std::ostringstream gc_cause;
  gc_cause << "try to allocate " << alloc_size << "byte";
  DumpGcForAllocInfo(LOG_STREAM(INFO), self, gc_cause.str());
#endif
...
#if defined(GC_DUMP)
  log_gc = (log_gc || gc_cause == kGcCauseForAlloc);
#endif
...
#if defined(GC_DUMP)
    DumpGcForAllocInfo(LOG_STREAM(INFO), self, "Allocation stack overflow", true);
#endif
...
void Heap::PushOnThreadLocalAllocationStackWithInternalGC(Thread* self,
  ...
  while (!allocation_stack_->AtomicBumpBack(kThreadLocalAllocationStackSize, &start_address,
    ...
    DumpGcForAllocInfo(LOG_STREAM(INFO), self,  "Thread local allocation stack overflow in " +
        thread_name, true);
#endif
    CollectGarbageInternal(collector::kGcTypeSticky, kGcCauseForAlloc, false);
  }
