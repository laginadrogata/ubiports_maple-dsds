...
#define mp_gfp_no_kswapd(gfp)	(gfp | __GFP_NO_KSWAPD)
#else
#define mp_gfp_no_kswapd(gfp)	(gfp & ~__GFP_KSWAPD_RECLAIM)
#endif
#ifdef __GFP_WAIT
#define mp_gfp_wait(gfp)		(gfp | __GFP_WAIT)
#define mp_gfp_not_wait(gfp)	(gfp & ~__GFP_WAIT)
#else
#define mp_gfp_wait(gfp)		(gfp | __GFP_RECLAIM)
#define mp_gfp_not_wait(gfp)	(gfp & ~__GFP_RECLAIM)
#endif
...
static gfp_t get_gfp_from_memory_prof_gfp(u64 mp_gfp)
{
	...
	if (mp_gfp & MP_GFP_NO_KSWAPD)
		gfp = mp_gfp_no_kswapd(gfp);
	if (mp_gfp & MP_GFP_WAIT)
		gfp = mp_gfp_wait(gfp);

	if (mp_gfp & MP_GFPNOT_WAIT)
		gfp = mp_gfp_not_wait(gfp);
